...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/29 12:12:38 by jheras-f          #+#    #+#             */
/*   Updated: 2019/12/19 15:26:13 by jheras-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(list->uval = va_arg(list->pl, unsigned int)) && list->prze > 0)
		list->aux = ft_atos('0');
	else if (list->prze == 0 && list->uval == 0 && list->prec == 1)
		list->aux = ft_strdup("");
	else
		list->aux = ft_utoa(list->uval);
